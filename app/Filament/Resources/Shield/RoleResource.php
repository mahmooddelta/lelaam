...
class RoleResource extends Resource {
		...
		return collect(Filament::getResources())
			->filter(function ($resource) {
				if ( config('filament-shield.exclude.enabled') ) {
					return !in_array(Str::of($resource)
						                 ->afterLast('\\'), config('filament-shield.exclude.resources'));
				}
			...
			$pages[] = Forms\Components\Grid::make()
				->schema([
					         Forms\Components\Checkbox::make($page)
						         ->label(Str::of($page)
							                 ->after(config('filament-shield.prefixes.page') . '_')
							                 ->headline())
						         ->inline()
	...
	protected static function getCustomEntities (): ?Collection {
		...
		return Permission::whereNotIn('name', $entitiesPermissions)
			->pluck('name');
	}
